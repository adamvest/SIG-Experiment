...
def train_linear_regression_model(data_x, data_y, lr, num_epochs, batch_size):
    ...
    for i in range(1, num_epochs+1):
        ...
        for j in range(num_batches):
            ...
            batch_loss = np.sum(.5 * np.square((diffs)))
            grad = sum([diffs[i] * batch_x[i] for i in range(batch_size)])
            weights -= lr * grad
...
if __name__ == "__main__":
    ...
    optimal_weights = train_linear_regression_model(train_x, train_y, args.lr, args.num_epochs, args.batch_size)
